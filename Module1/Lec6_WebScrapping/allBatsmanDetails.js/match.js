...
function  processDetails(teamName , batsmanName , runs , balls , fours , sixes , strikeRate){
    ...
    // boolean check=IsTeamPresent(teamName);
    ...
    let oneBatsamanDetails={Runs:runs,
                 Balls:balls,
                 Fours :fours,
                 Sizes :sixes,
    ...
    let batsmanPath=teamPath+"/" + batsmanName;

    if(!fs.existsSync(batsmanPath)){
        
        let batsmanData=[];
        batsmanData.push(oneBatsamanDetails);
        fs.writeFileSync(batsmanPath,JSON.stringify(batsmanData));
    }
    else{
        let readBData=fs.readFileSync(batsmanPath);
        readBData=JSON.parse(readBData);

        readBData.push(oneBatsamanDetails);
        readBData=JSON.stringify(readBData);
        fs.writeFileSync(batsmanPath,readBData);
    }
}
